...
def send_email(subject, body):
    sender_email = "soylu.aerospace@gmail.com"
    receiver_email = "burak.gnndd@gmail.com"
    password = "thnh wyip xjrc dlcc"  # Gmail için uygulama şifresi burada girilmeli
    ...
    try:
        # Gmail'in SMTP sunucusu üzerinden mail gönderme
    ...
    if not tracking_started:  # Eğer takip başlamamışsa, YOLO ile nesne tespiti yap
        ...
        if success:
            ...
            if cv2.pointPolygonTest(polygon, bbox_center, False) >= 0:
                ...
                # Mail gönderme (poligona girildiğinde)
                if not in_polygon:
                    send_email("İzinsiz Giriş Tespit Edildi", "Alana izinsiz giriş tespit edilmiştir.")
                    in_polygon = True
                ...
                if in_polygon:
                    ...
                    # Mail gönderme (poligondan çıkıldığında)
                    send_email("İzinsiz Giriş Sonlandırıldı", "Alana izinsiz giriş sonlandırılmıştır.")
                    in_polygon = False
        else:
